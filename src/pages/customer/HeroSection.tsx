            ...
            {data.map((_, i) => (
              <div
                ...
                className={`cursor-pointer rounded-full transition-all duration-300 ${
                  index === i + 1 ? "w-20 h-2 bg-primary" : "w-3 h-2 bg-white/60"
                }`}
              ></div>
            ))}
          </div>
...
const DefaultLandingPage = () => (
  <div className="relative min-h-screen w-full flex items-center justify-center bg-black text-white">
    <div className="w-full h-screen relative">
      <img className="w-full h-full object-cover object-[calc(50%+82px)_center] xl:object-center" src={theater} alt="" />

      <img src={textImage} alt="" className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" />
    </div>
  </div>
);
